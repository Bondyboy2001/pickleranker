...
function parseWeeklySnapshot(
  html: string,
  when: string,
  playerId: (name: string) => string,
): WeeklySnapshot {
  const players: WeeklySnapshot['players'] = []
  const rowRegex = /<tr>\s*<th[^>]*>(\d+)<\/th>\s*<td[^>]*>([\s\S]*?)<\/td>\s*<td[^>]*>\s*<strong>([\d.]+)<\/strong>\s*<\/td>\s*<td[^>]*>[\s\S]*?<\/td>\s*<td[^>]*>([\s\S]*?)<\/td>/g
  let match: RegExpExecArray | null
...
async function main() {
  ...
  for (const when of weeks) {
    ...
    weeklySnapshots.push(parseWeeklySnapshot(leaderboardHtml, when, getPlayerId))
...
export const cardiffSeedData = ${JSON.stringify({ players, matches, weeklySnapshots }, null, 2)}
`
